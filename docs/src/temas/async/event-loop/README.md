...
* There is in fact a [Render queue](render-queue), which is another queue in the event loop system which basically takes care of all tasks to be done before every screen update or repaints.
